...
const KataPengantar = () => {
    ...
    return (
        <div className={contentContainerClasses}>
            ...
            <p className={`${paragraphClasses}`}>Di dalamnya, Anda tidak hanya akan membaca tentang hati, akal, intuisi, atau metode Kawrooh. Anda akan diajak mengalami—merenung, melepaskan, menyambung ulang. Bukan sekadar teori. <b>Ini Ebook untuk dipraktikkan, bukan hanya dikoleksi.</b> Semoga setiap bab-nya membawa Anda <b>lebih dekat pada rasa tenteram, lebih jujur pada diri sendiri, lebih terbuka pada limpahan karunia-Nya,Sehingga Pintu Rejeki Terbuka, dan Hidup Penuh Keberkahan</b>. Selamat menyelami.</p>
...
<li><button onClick={() => setCurrentPageKey('bab14')} className={tocSectionClasses}> Bagian VII: Spiritualitas dan Hukum Kehidupan</button>
                    ...
                    <li className="pt-4"><button onClick={() => setCurrentPageKey('pixel-thoughts')} className="text-yellow-600 hover:underline font-bold text-xl">✨ Ruang Pelepasan (Lepaskan Beban)✨</button></li>
                  <li className="pt-2"><button onClick={()=>setCurrentPageKey('affirmation-room')} className="text-sky-500 hover:underline font-bold text-xl">✨ Ruang Afirmasi (Isi Energi Positif)✨</button></li>
...
const PixelThoughts = () => {
  ...
  const shareableRef = useRef(null); // Untuk 'div' yang akan jadi gambar
  ...
  const handleShare = async (platform) => {
    ...
    } else { // Fallback untuk copy atau desktop
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 text-white flex flex-col justify-start items-center p-4 pt-20">
      ...
      <div className="absolute top-4 right-4 z-20">
        <button onClick={() => setCurrentPageKey('daftar-isi')} className="bg-white/20 px-4 py-2 rounded-lg hover:bg-white/30 transition-colors">&larr; Kembali</button>
      </div>
      <div className="z-10 w-full max-w-2xl text-center flex flex-col items-center">
        <div className="w-full h-48 flex flex-col justify-center items-center">
          ...
          {view === 'finished' && (
            <div className="animate-fade-in w-full">
              <h2 className="text-2xl md:text-4xl font-bold mb-4">Bagaimana Perasaan Kamu Sekarang?</h2>
              <input
                type="text"
                value={feeling}
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-3">
                <button onClick={() => handleShare('native')} className="bg-blue-500 text-white font-bold px-5 py-2 rounded-lg hover:bg-blue-600 flex items-center gap-2">Bagikan</button>
                <button onClick={handleDownloadImage} className="bg-purple-600 text-white font-bold px-5 py-2 rounded-lg hover:bg-purple-700 flex items-center gap-2">Download Story</button>
              </div>
              <div className="mt-10 flex flex-col md:flex-row gap-4 justify-center">
                <button onClick={handleRestart} className="bg-white/20 px-6 py-3 rounded-lg hover:bg-white/30 transition-colors">Ulangi Sesi</button>
